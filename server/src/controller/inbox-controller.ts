import { Inbox, Notification, NotificationTypes } from "../../../shared/Notification";
    ...
    try {
        ...
        const inbox: Inbox = {
            activity: {
                ...
                has_read: activity.length > 0 && activity.every(x=>x.has_read),
            },
            announcement: {
                count: announcement.length,
                has_read: announcement.length > 0 && announcement.every(x=>x.has_read),
            },
            assignment: {
                count: assignment.length,
                has_read: assignment.length > 0 && assignment.every(x=>x.has_read),
            }
        }
